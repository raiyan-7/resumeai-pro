...
export const Sidebar = ({ isCollapsed, setIsCollapsed }) => {
  ...
  return (
    <aside
      className={`h-screen sticky top-0 bg-slate-950/80 border-r border-slate-900 backdrop-blur-md transition-all duration-300 flex flex-col z-20 ${
    ...
    >
      ...
      <div className="h-16 px-6 border-b border-slate-900 flex items-center justify-between">
        ...
        <button
          ...
          className="hidden md:flex p-1.5 rounded-lg border border-slate-800 text-slate-400 hover:text-slate-200 hover:bg-slate-900 transition-colors shrink-0"
        >
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${
                isActive
                  ? 'bg-gradient-to-r from-brand-500/10 to-accent-500/5 text-brand-400 border border-brand-500/20'
      ...
      <div className="p-4 border-t border-slate-900 flex flex-col gap-2 shrink-0">
        {!isCollapsed && (
          <div className="flex items-center gap-3 px-2 py-1 bg-slate-900/40 rounded-xl border border-slate-900/50">
            <div className="w-8 h-8 rounded-full bg-slate-800 flex items-center justify-center font-bold text-accent-400 shrink-0 border border-slate-700/50">
              {user?.full_name ? user.full_name.charAt(0).toUpperCase() : 'U'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-semibold text-slate-200 truncate">{user?.full_name || 'ResumeAI Member'}</p>
        ...
        <button
          ...
          className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-rose-400 hover:text-rose-300 hover:bg-rose-950/20 border border-transparent w-full transition-colors`}
        >
